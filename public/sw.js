...
// Fetch event - intelligent caching strategy (single handler)
self.addEventListener("fetch", (event) => {
  const { request } = event;
  const url = new URL(request.url);

  // Guard: only handle same-origin requests to avoid interfering with dev/HMR
...
// (Removed duplicate fetch performance wrapper to prevent double respondWith)
